...
const CuteInterface = ({ palData, onExplore, wallet, onOpenSettings, onUpdateStats }) => {
    ...
    return (
        <div className={`w-full h-screen bg-gradient-to-b ${bgGradient} transition-colors duration-1000 flex items-center justify-center font-quicksand text-gray-700 overflow-hidden relative`}>
            ...
            <div className="absolute top-10 left-10 text-white/40 animate-cute-float" style={{ animationDelay: '0s' }}><Cloud size={64} /></div>
            <div className="absolute top-20 right-10 text-white/40 animate-cute-float" style={{ animationDelay: '1.5s' }}><Cloud size={48} /></div>
            ...
            <div className="relative z-10 w-full max-w-sm h-screen md:h-[85vh] md:max-w-6xl md:aspect-[16/9] bg-white/60 backdrop-blur-xl md:rounded-[3rem] shadow-2xl md:border-8 border-white p-6 md:p-8 flex flex-col md:flex-row justify-between transition-all duration-500">
                ...
                <div className="flex-1 flex flex-col items-center justify-center relative md:w-5/12 order-2 md:order-1">
                    <div
                        className={`text-[150px] md:text-[220px] filter drop-shadow-xl cursor-pointer select-none transition-transform ${isBouncing ? 'animate-squish' : 'animate-cute-float'}`}
