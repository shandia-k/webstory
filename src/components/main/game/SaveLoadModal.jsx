...
export function SaveLoadModal({ isOpen, onClose, onSaveGame, onLoadGame, onExitToMenu }) {
    ...
    useEffect(() => {
        ...
        // Set initial focus to Close button after transition
    ...
    return (
        ...
        >
            <div
                ref={modalRef}
                className={`bg-white/95 backdrop-blur-xl border-4 border-white/50 rounded-[2.5rem] p-6 md:p-8 w-full max-w-lg shadow-2xl relative overflow-hidden ring-4 ring-black/5 transition-all duration-300 ease-out delay-75 ${isOpen ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-8 opacity-0'}`}
            >
