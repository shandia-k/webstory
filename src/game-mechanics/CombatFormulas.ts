...
export const calculateStats = (baseStats, currentStats, traitId) => {
    ...
    // ATK: Priority to STORED stat, fallback to formula
    const ATK = currentStats.atk || (15 + (level * 2));

    // DEF: Priority to STORED stat, fallback to formula
    const DEF = currentStats.def || (10 + (level * 1));
