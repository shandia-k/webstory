...
export const generatePalChat = async (apiKey, palData, userMessage, history, genre) => {
    ...
    try {
        // SECURITY: Validate user input to prevent massive prompts or control char injection
        const cleanMessage = validateInput(userMessage, 1000);
        const recentHistory = history.slice(-5).map(h => `${h.role === 'user' ? 'Human' : palData.name}: ${validateInput(h.text, 500)} `).join('\n');
